...
int main(void)
{
	...
	while(1)
	{
		...
		while(I2C_MasterReceiveDataIT(&I2C1Handle, &rxLen, 1, SLAVE_ADDR, I2C_ENABLE_SR) != I2C_READY);
		// Wait until RX is complete before continuing
		...
		while(I2C_MasterReceiveDataIT(&I2C1Handle, rxBuffer, rxLen, SLAVE_ADDR, I2C_DISABLE_SR) != I2C_READY);

		// Wait until RX is complete before printing
		while(rxComplete != SET);

		// Terminate string with the null character and print
		rxBuffer[rxLen + 1] = '\0';
...
void I2C_ApplicationEventCallback(I2C_Handle_t *pI2CHandle, uint8_t appEv)
{
	...
	{
		printf("ACK failure has occurred!\n");

		// In Master mode ACK failure happens when the Slave fails to send ACK after the Master sends a byte
